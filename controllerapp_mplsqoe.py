...
class slicing:
    ...
    def slicingqoe(self):
        ...
        self.manager.start()
        if self.bitrate<1000000.0 or self.buffer_size<5:
            print("done")
          ##3 QoS Setting to install flow entry
            # subprocess.call('curl -X POST -d \'{\"port_name\": \"s2-eth2\", \"type\": \"linux-htb\", \"max_rate\": \"3000000\", \"queues\": [{\"max_rate\": \"3000000\"}, {\"min_rate\": \"2500000\"}]}\' http://localhost:8080/qos/queue/0000000000000002',shell=True)
            # subprocess.call('curl -X POST -d \'{\"port_name\": \"s2-eth3\", \"type\": \"linux-htb\", \"max_rate\": \"1500000\", \"queues\": [{\"max_rate\": \"1500000\"}, {\"min_rate\": \"1000000\"}]}\' http://localhost:8080/qos/queue/0000000000000002',shell=True)
            ...
            # subprocess.call('curl -X POST -d \'{\"match\": {\"nw_src\": \"10.0.0.2\", \"nw_proto\": \"TCP\", \"tp_dst\": \"80\"}, \"actions\":{\"queue\": \"1\"}}\' http://localhost:8080/qos/rules/0000000000000002',shell=True)
